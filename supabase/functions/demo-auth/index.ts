...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Parse body safely - some actions don't send a body
    ...
    try {
      ...
      // No body or invalid JSON is fine for list/seed
    }
    ...
    switch (action) {
      ...
      case "login":
        return await handleLogin(body as { login: string; password: string });
      case "reset-password":
        return await handleResetPassword(body as { account_id: string; new_password: string });
      case "toggle":
        return await handleToggle(body as { account_id: string; enabled: boolean });
      case "seed":
